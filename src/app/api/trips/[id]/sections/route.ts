...
type TripDateRange = { startDate: Date; endDate: Date };
type ReorderSegment = { id: number; position: number };
type PopulateSegment = {
  id: number;
  type: string;
  title: string;
  description: string;
...
type GeneratedReturn = { id: number };
...
function invalidTripDate(date: Date, range: TripDateRange) {
...
export async function PATCH(
  ...
  if (body.action === "overview") {
    ...
    const segments: ReorderSegment[] = await prisma.travelSegment.findMany({
      where: { tripId },
      orderBy: [{ position: "asc" }, { id: "asc" }],
      select: { id: true, position: true },
    });
    const index = segments.findIndex((segment: ReorderSegment) => segment.id === itemId);
    ...
    const [trip, segments, itineraryCount]: [TripDateRange | null, PopulateSegment[], number] = await Promise.all([
    ...
    const invalidSegment = segments.find((segment: PopulateSegment) => segment.date && !isDateWithinTrip(segment.date, trip.startDate, trip.endDate));
    if (invalidSegment) {
    ...
    await prisma.$transaction(async (tx: TransactionClient) => {
      ...
      await tx.itineraryItem.createMany({
        data: segments.map((segment: PopulateSegment) => ({
          tripId,
  ...
  const result = body?.action === "itinerary"
    ...
    : body?.action === "segment" && segment
      ? await prisma.$transaction(async (tx: TransactionClient) => {
          ...
          const generatedReturns: GeneratedReturn[] = await tx.travelSegment.findMany({ where: { tripId, returnForSegmentId: itemId, autoGenerated: true }, select: { id: true } });
          const removedIds = [itemId, ...generatedReturns.map((item: GeneratedReturn) => item.id)];
