...
import { TRAVEL_SEGMENT_TYPES } from "@/lib/trips";
...
type Notice = { type: "success" | "warning" | "error"; text: string; scope: NoticeScope };
...
export function TripSections(props: TripSectionsProps) {
  ...
  async function jsonRequest(method: string, body: Record<string, unknown>, successMessage: string, scope: NoticeScope = "global") {
    setNotice(null);
    ...
    try {
      ...
      setNotice({ type: "success", text: successMessage, scope });
      router.refresh();
      return true;
    } catch (requestError) {
      setNotice({ type: "error", text: requestError instanceof Error ? requestError.message : "Unable to save changes", scope });
      return false;
    } finally {
      setPendingAction("");
    }
  ...
  async function submitForm(event: FormEvent<HTMLFormElement>, action: string, method = "PATCH", successMessage?: string) {
    ...
    const formFields = fields(event.currentTarget);
  ...
  return (
    <div className="mt-10 grid gap-8">
      ...
      <section className="order-3 border rounded-lg p-6" id="itinerary">
        ...
        {notice?.scope === "itinerary" && <Notification className="mb-4" message={notice.text} type={notice.type} />}
      ...
      <section className="order-2 border rounded-lg p-6" id="travel">
        ...
        {notice?.scope === "travel" && <Notification className="mb-4" message={notice.text} type={notice.type} />}
