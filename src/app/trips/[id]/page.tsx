  ...
  return (
    <main className="min-h-screen p-8 max-w-4xl mx-auto">
      ...
      <TripSections
        ...
        notes={trip.notes}
        photos={trip.photos}
        updatesSection={<TripUpdates itineraryItems={trip.itineraryItems.map((item) => ({ id: item.id, label: `${item.date.toISOString().slice(0, 10)} ${item.time} - ${item.title}` }))} travelSegments={trip.travelSegments.map((item) => ({ id: item.id, label: `${item.type} - ${item.title}` }))} tripId={trip.id} updates={trip.updates.map((update) => ({ id: update.id, title: update.title, content: update.content, createdAt: update.createdAt.toISOString(), updateType: update.updateType as "GENERAL" | "TRAVEL" | "ITINERARY", travelSegmentId: update.travelSegmentId, itineraryItemId: update.itineraryItemId }))} />}
